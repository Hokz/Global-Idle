...
> green on **every run since the first green one** — nine consecutive, and on both jobs
> for the seven since `dev-bootstrap` was added
> ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation), most recently [35616431594](https://github.com/Hokz/Global-Idle/actions/runs/35616431594)). The FIRST green run took three attempts
...
| 7 | Integration tests against ephemeral PostgreSQL and Redis | **satisfied.** 84 integration and 6 invariant cases pass **under Testcontainers in CI** ([run 35616431594](https://github.com/Hokz/Global-Idle/actions/runs/35616431594), checks 10 and 11), and locally against PostgreSQL 16 and Redis through the documented escape hatch |
...
| 19 | **CI green, thirteen checks, each the same script a developer runs** | **satisfied.** All thirteen green on GitHub for **nine consecutive runs** ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation); most recently [35616431594](https://github.com/Hokz/Global-Idle/actions/runs/35616431594)). The same thirteen also pass locally, in the same order, against a genuinely clean checkout |
...
| Claim | Status |
|---|---|
| **CI is green** (§16 criterion 19) | **VERIFIED** — all thirteen checks plus the matrix count, green for **nine consecutive runs** ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation); most recently [35616431594](https://github.com/Hokz/Global-Idle/actions/runs/35616431594)). The two runs before those failed, which is how i25, i26 and i27 were found. The first green run took three attempts; those three defects are i25, i26 and i27 |
