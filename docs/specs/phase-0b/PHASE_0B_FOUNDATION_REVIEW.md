...
> green on **every run since the first green one**, and on both jobs since `dev-bootstrap`
> was added ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation) are the live
> state; a count here would be wrong one commit later). The FIRST green run took three attempts
...
| 7 | Integration tests against ephemeral PostgreSQL and Redis | **satisfied.** 84 integration and 6 invariant cases pass **under Testcontainers in CI** ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation), checks 10 and 11), and locally against PostgreSQL 16 and Redis through the documented escape hatch |
...
| 19 | **CI green, thirteen checks, each the same script a developer runs** | **satisfied.** All thirteen green on GitHub on **every run since the first green one** ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation)). The same thirteen also pass locally, in the same order, against a genuinely clean checkout |
...
| Claim | Status |
|---|---|
| **CI is green** (§16 criterion 19) | **VERIFIED** — all thirteen checks plus the matrix count, green on **every run since the first green one** ([the branch's runs](https://github.com/Hokz/Global-Idle/actions?query=branch%3Afeat%2Fphase-0b-technical-foundation)). The two runs before it failed, which is how i25, i26 and i27 were found. The first green run took three attempts; those three defects are i25, i26 and i27 |
