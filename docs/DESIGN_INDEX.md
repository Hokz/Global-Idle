...
| `docs/specs/pre-phase-4/PRE_PHASE_4_SPEC.md` | **PRE-PHASE-4 specification** — the gate between Phase 3.7 and Phase 4, not a Phase 3.8: the Login above its Game Accounts, the Game Account deletion lifecycle with its freeze, restore, archive and purge (G4.1), the XP → Level projection (G4.2), the Actor / Participant contract (G4.3), globally unique case-insensitive names (G4.4) and the tunable configuration surface (G4.5), with a 109-case matrix. `IMPLEMENTATION_SPEC_DRAFT` — **pending independent review; nothing of it is implemented** |
