...
| [023](decisions/ADR-023-quest-replay-and-one-time-reward-claims.md) | Quest replay is separate from one-time reward claims; co-op quests can be replayed, and a one-time reward is claimed once per Game Account, whichever actor | `ACCEPTED` — product rules `LOCKED` 2026-09-25; corrected after the review of `e2d0e04`, **pending independent review**; **not implemented** (Phase 5) |
